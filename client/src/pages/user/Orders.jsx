        ...
        <div className="m-20">
          ...
          <Table aria-label='simple-table' className="">
            <TableHead className="border-b-2 border-orange-500">
              <TableRow>
                <TableCell><span className="font-varela font-bold text-lg">#</span></TableCell>
                <TableCell><span className="font-varela font-bold text-lg">Status</span></TableCell>
                <TableCell><span className="font-varela font-bold text-lg">Buyer</span></TableCell>
                <TableCell><span className="font-varela font-bold text-lg">Ordered</span></TableCell>
                <TableCell><span className="font-varela font-bold text-lg">Payment</span></TableCell>
                <TableCell><span className="font-varela font-bold text-lg">Quantity</span></TableCell>
              </TableRow>
            </TableHead>
          ...
          {o?.products?.map((p,i) => (
            <Box key={i} className='mx-auto flex items-center p-5'>
              <Box className=' border-b-2 border-orange-500  w-full p-5 flex items-center space-x-5'>
                <img src= {`${import.meta.env.VITE_APP_REACT_APP_API}/product/photo/${ p._id}`} alt="" className="w-32"/>
              

              <Box className='flex space-x-10 mx-auto justify-center'>
                <Box>
                  <h1 className="text-gray-500">Product</h1>
                  <span className="text-2xl font-varela font-bold">{p.name}</span>
                </Box>
                <Box>
                  <h1 className="text-gray-500 font-varela">Price</h1>
                  <span className="text-2xl font-varela font-bold">{p?.price?.toLocaleString("en-Us", {
                    style: "currency",
                    currency: "PHP",
                  })}</span>
                </Box>
